...
public class MailedCaseCI extends TestBase {

    private static final Logger logger = Logger.getLogger(MailedCaseCI.class.getName());
    ...
    public void createManualCase() {
        try {
            ...
            inspectorNameType1.sendKeys("New Stage Inspector");
            Thread.sleep(3000);

            List<WebElement> suggList1 = driver.findElements(By.cssSelector(OR.getProperty("InspSelectMail")));
            Thread.sleep(1000);
            for (WebElement e8 : suggList1) {
                if (e8.getText().equalsIgnoreCase("New Stage Inspector")) {
                    e8.click();
            ...
            Thread.sleep(2000);
            ExtentReporterNG.addStep("Collective Invoice. selected");
